...
def assert_package_edition_metadata(manifest: dict[str, Any]) -> None:
    ...
    registry_match = re.search(
        r"(?m)^- `([^`]+)` — the ([0-9]+)-rule draft\.2 registry\.$", readme
    )
...
def main() -> int:
    ...
    for rule in rules:
        ...
        assert rule["rule_version"] in ("1.0.0-draft.1", "1.0.0-draft.2"), rule["rule_id"]
    ...
    # Only amended or new rules carry the draft.2 rule_version.
