...
- `rules/ats_rules_v1.yaml` — normative rule registry and default rule states;
...
The machine-readable registry is in `rules/ats_rules_v1.yaml`.
...
| `ATS-DISC-003` | A restatement SHOULD add scope, evidence, mechanism, implication, contrast, action, or retrieval value. | Advisory | Advisory | D3 |
...
The profile validators in Section 9 operate in addition to the thirty text rules. An artifact can pass every individual local rule and still fail profile completeness because a required section-level semantic slot is absent.
